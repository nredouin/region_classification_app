...
def show_swatch(region: int, size: int):
    img_path = SWATCH_DIR / f"{region}.jpg"
    if img_path.exists():
...
if "df" not in st.session_state:
    if Path(EXCEL_PATH).exists():
        ...
        st.session_state.df = pd.DataFrame(columns=["Region", "Reflet 1", "Reflet 2", "Famille Lp Name", "DMI Name"])
...
with st.sidebar:
    ...
    uploaded = st.file_uploader("Load Excel (from app.py export or previous session)", type=["xlsx"])
    ...
    if uploaded is not None and st.session_state.get("_last_upload_id") != upload_id:
        ...
        if "Kept Regions" in xl.sheet_names:
            ...
            # Single-sheet file from app.py export — simple load, no selections restored
    ...
    if not st.session_state.df.empty:
        ...
        st.download_button(
            ...
            data=buf_exp,
            file_name="labeled_regions_in_export.xlsx",
...
if st.session_state.df.empty:
    st.title("India Region Label Analyzer")
    st.info("Upload the Excel export from app.py using the sidebar to get started.")
    st.stop()
...
with tab2:
    ...
    if not kept_set:
        ...
        st.download_button(
            ...
            data=buf2,
            file_name="region_mapping_in.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )
